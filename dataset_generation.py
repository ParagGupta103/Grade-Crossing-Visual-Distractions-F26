...
print(
    f"Starting High-Coverage sweep for {len(df)} locations (Total expected images: {total_expected})..."
)
...
for index, row in df.iterrows():
    ...
    for current_heading in range(0, 360, 45):
        ...
        if response.status_code == 200:
            ...
            print(
                f"[{index + 1}/{len(df)}] Saved {lat}, {lng} facing {current_heading}°"
            )

        elif response.status_code == 404:
            print(
                f"[{index + 1}/{len(df)}] No imagery found at {lat}, {lng} for {current_heading}° (404)."
            )
